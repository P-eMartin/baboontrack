...
class MyClassifier:
    ...
    def extract_feature(self, img):
        ...
        if self.det is not None:
            ...
            x1, y1 = int(min(max(0, np.floor(x1)), image.shape[1]-1)), int(min(max(0, np.floor(y1)), image.shape[0]-1))
            x2, y2 = max(x1+1, int(min(image.shape[1], np.ceil(x2)))), max(y1+1, int(min(image.shape[0], np.ceil(y2))))
            ...
            for path in paths:
                ...
                if feature is None:
                    ...
                    if bbox is not None:
                        ...
                        ref_path = os.path.join('.tmp', 'ref_crops', '%s_%s_%g_%g_%g' % (self.name_database, self.detector_type, self.det_thr, self.nms_thr, self.roi_det), class_id, os.path.basename(path))
