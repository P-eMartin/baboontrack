...
class PrimateFace:
    ...
    def __init__(
        ...
        self._processor = PrimateFaceProcessor(
            ...
            pose_config=str(pose_cfg) if pose_cfg is not None else None,
            pose_checkpoint=str(pose_ckpt) if pose_ckpt is not None else None,
            device=self.device,
        )
    ...
    def analyze(
        ...
        print(f"Detected {len(bboxes)} faces in the image.")

        if len(bboxes) == 0:
            return []

        # Run pose estimation
        if self._processor.pose_model is not None:
        ...
        for i in range(len(bboxes)):
            if self._processor.pose_model is not None:
                ...
                kpts_with_vis = np.zeros((0, 3), dtype=np.float32)
