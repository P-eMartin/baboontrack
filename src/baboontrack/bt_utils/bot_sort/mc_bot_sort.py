...
from .matching import iou_distance, fuse_score, embedding_distance, linear_assignment
from .gmc import GMC
...
class BoTSORT(object):
    def __init__(self, track_high_thresh=0.7, track_low_thresh=0.1, new_track_thresh=0.7, match_thresh=0.8, proximity_thresh=0.5, buffer_size=60, appearance_thresh=0.25, cmc_method='sparseOptFlow', verbose_cmc=None, ablation=False,encoder=None, mot20=False, frame_rate=30):
        ...
        self.encoder = encoder #FastReIDInterface(args.fast_reid_config, args.fast_reid_weights, args.device)

        self.gmc = GMC(method=cmc_method, verbose=verbose_cmc) # method=args.cmc_method, verbose=[args.name, args.ablation]
    ...
    def update(self, output_results, img):
        ...
        if self.encoder is not None:
            ...
            # dists = fuse_motion(self.kalman_filter, raw_emb_dists, strack_pool, detections)
            # emb_dists = dists
